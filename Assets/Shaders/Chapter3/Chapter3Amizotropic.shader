Shader "CookbookShaders/Chapter3/Chapter3Amizotropic"
{
	Properties
	{
		...
		_Specular		("Cpecular Amount", Range(0, 1))		= 0.5
	...
	SubShader
	{
		...
		inline fixed4 LightingAnisotropic(SurfaceAnisoOutput s, fixed3 lightDir, half3 viewDir, fixed atten)
		{
			...
			fixed NdotA = dot(normalize(s.Normal + s.AnisoDirection), halfVector);

			float aniso = max(0, sin(radians((NdotA + _AnisoOffset) * 180.0f)));
